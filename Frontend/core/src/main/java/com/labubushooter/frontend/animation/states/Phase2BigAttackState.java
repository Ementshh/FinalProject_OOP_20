...
/**
 * Phase 2 big attack preparation animation state for Final Boss (static).
 * Displays a single texture when boss is preparing for big attack.
 * Replaces the yellow flashing box warning.
 * 
 * Design Pattern: State Pattern
 * - Implements BossAnimationState for Phase 2 big attack preparation behavior
 * - Static state (no animation)
 * 
 * SOLID Principles:
 * - Single Responsibility: Only handles Phase 2 big attack preparation display
 * - Open/Closed: Can be extended without modifying other states
 */
public class Phase2BigAttackState implements BossAnimationState {
    
    private final Texture bigAttackTexture;
    
    /**
     * Creates a new Phase 2 big attack preparation animation state.
     * 
     * @param bigAttackTexture The big attack preparation texture (boss_phase2_bigattack.png)
     */
    public Phase2BigAttackState(Texture bigAttackTexture) {
