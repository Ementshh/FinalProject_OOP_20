...
public class CollisionSystem {
    ...
    public void checkPickupPlayerCollisions(Player player, Array<Pickup> pickups, Pool<Pickup> pickupPool,
                                            PistolStrategy pistolStrategy, Mac10Strategy mac10Strategy) {
        for (int i = pickups.size - 1; i >= 0; i--) {
            Pickup p = pickups.get(i);
            if (p.active && p.bounds.overlaps(player.bounds)) {
                switch (p.type) {
                    case AMMO_9MM:
                        if (pistolStrategy != null) {
                            pistolStrategy.addAmmo(3);
                            Gdx.app.log("Pickup", "Collected 9mm Ammo (+3)");
                        }
                        break;
                    case AMMO_45CAL:
                        if (mac10Strategy != null) {
                            mac10Strategy.addAmmo(5);
                            Gdx.app.log("Pickup", "Collected .45 Cal Ammo (+5)");
                        }
                        break;
                    case HEALTH_POTION:
                        player.addHealth(5);
                        Gdx.app.log("Pickup", "Collected Health Potion (+5 HP)");
                        break;
                }
                p.active = false;
