...
/**
 * Enemy spawner for Level 5 (Final Boss level).
 * No regular enemy spawning - final boss fight only.
 * 
 * Design Pattern: 
 ...
 * 
 * SOLID Principles:
 * - Single Responsibility: Level 5 (no-spawn) behavior only
 * - Liskov Substitution: Safe to use where IEnemySpawner expected
 */
public class Level5Spawner extends BaseEnemySpawner {
    
    @Override
    public int getMaxEnemies() {
        return 0; // No regular enemies in final boss level
    }
    ...
    public boolean isActive() {
        return false; // Final boss level - no regular enemy spawning
    }
    ...
    public boolean trySpawnEnemy(Pool<CommonEnemy> enemyPool, Array<CommonEnemy> activeEnemies,
        ...
        return false; // Never spawn in final boss level
    }
}
